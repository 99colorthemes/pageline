...
class Pageline_Project_Widget extends WP_Widget {
  ...
  public function form( $instance ) {
    ...
    <p><label for="<?php echo $this->get_field_id('btn_text'); ?>"><?php esc_html_e( 'Button Text:', 'pageline' ); ?></label>
...
class Pageline_blogs_Widget extends WP_Widget {
  ...
  public function form( $instance ) {
    ...
    <p><label for="<?php echo $this->get_field_id('btn_txt'); ?>"><?php esc_html_e( 'Button Text:', 'pageline' ); ?></label>
    <input class="widefat" id="<?php echo $this->get_field_id('btn_txt'); ?>" name="<?php echo $this->get_field_name('btn_txt'); ?>" type="text" value="<?php echo esc_attr($btn_txt); ?>" /></p>
        ...
        <div class="nnc-testimonial">
          ...
          <?php for ($i=0; $i<3; $i++) : ?>
            <div class="testimonial item">
              ...
              <?php endif; ?>
                <?php if ( !empty( $image[$i] ) ) { echo '<img src='.esc_url( $image[$i] ).' alt='.esc_attr( $name[$i] ).'>'; } ?>
            </div>
          <?php endfor; ?>
        </div>
        </div>
