...
function pageline_posted_on() {
	...
	<?php if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) { ?>
		<span class="comments-link"><i class="fa fa-comments" aria-hidden="true"></i> <?php comments_popup_link( __( '0', 'pageline' ), __( '1', 'pageline' ), __( ' % Comments', 'pageline' ) ); ?></span>
	<?php }
...
function pageline_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && pageline_categorized_blog() ) {
			printf( '<span class="cat-links"><i class="fa fa-folder" aria-hidden="true"></i>' . esc_html__( '%1$s', 'pageline' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ' / ', 'pageline' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links"><i class="fa fa-tags" aria-hidden="true"></i>' . esc_html__( '%1$s', 'pageline' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
